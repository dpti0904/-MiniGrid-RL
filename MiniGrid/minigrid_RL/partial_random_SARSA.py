...
def train_sarsa_wall_random(num_episodes=1000,
                alpha=0.15, gamma=0.99,
                epsilon_start=1.0, epsilon_decay=0.999, epsilon_min=0.01):
    """
    Train SARSA with randomized wall layouts but fixed agent/goal positions.
    """
    # Setup environment
    ...
    for ep in range(num_episodes):
        # Reset without seed to randomize wall layout
        obs, _ = env.reset()
...
if __name__ == "__main__":
    ...
    print("WALL RANDOM SARSA with Randomized Wall Layouts")
    print("=" * 50)
    ...
    for t in range(num_trials):
        ...
        _, rewards_list = train_sarsa_wall_random(
    ...
    plt.title(f"WALL RANDOM SARSA: Average Reward per Episode (averaged over {num_trials} trials)")
    ...
    plt.title(f"WALL RANDOM SARSA: Average 100-Episode Success Rate (over {num_trials} trials)")
    plt.ylim(0, 1.05)
    plt.grid(True, linestyle="--", alpha=0.5)
    plt.tight_layout()
    plt.show()
    ...
    print("WALL RANDOM SARSA RESULTS:")
    print("=" * 30)
    ...
    print(f"- Environment seed: None (RANDOMIZED)")
